module MockGame = struct
  ...
  let evaluate state =
    float_of_int state (* Simply return the state as the evaluation *)

  let generate_actions state = if state <= 0 then [] else [ 1; 2; 3 ]
  (* Generate actions 1, 2, or 3 if state > 0 *)

  let apply_action state action =
    state - action (* Apply the action by subtracting it from the state *)
end
...
let test_minimax_terminal _ =
  ...
  (* Terminal state, should evaluate to 0.0 *)
...
let test_minimax_simple _ =
  ...
  assert_equal action (Some 3) ~printer:(fun a ->
      match a with Some a -> string_of_int a | None -> "none")
...
let test_search _ =
  ...
  assert_equal action
    1 (* The best action according to minimax should be action 1 *)
...
let test_minimax_minimizing _ =
  ...
  let alpha = neg_infinity in
  let beta = infinity in
  let maximizing = false in
  (* Minimizing player *)
  let value, action = AlphaBeta.minimax state depth alpha beta maximizing in
  (* Expected value is the minimal one from the next actions *)
  assert_equal value 0.0;
  (* The minimizing player will try to go to state 0, which is terminal *)
  assert_equal action (Some 3)
(* The minimizing player will pick action 3 to reach terminal state 0 *)
