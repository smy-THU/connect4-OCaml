...
type state = {
  ...
  bonus_point : point_t;
  get_bonus : int;
      (* 0 for not getting bonus yet, 1 for has getting bonus and is placing the ban, 2 for has placed ban point*)
}

(* Create the initial state with specified dimensions *)
let initial_state (h : int) (w : int) (player : player_t)
    (bonus_point : point_t) : state =
  ...
  if h < 6 || h > 12 || w < 6 || w > 12 then failwith "board size invalid"
    ...
    (* bonus_point is -2 on the board*)
    {
      ...
      last_move = (-1, -1);
      ban_point = (-1, -1);
      (* initial state when ban ponit is not placed yet*)
      bonus_point;
      get_bonus = 0;
      (* when initial, the bonus point has not been gotten by any player*)
    }

(* Check if the board is full, using the state *)
let is_full_state (state : state) : bool =
  let h = Utils.get_h state.board in
  if h < 2 then failwith "the board is invalid."
  else
    match state.get_bonus with
    | 0 -> Array.for_all (fun ele -> ele <> 0 && ele <> -2) state.board.(0)
    | 1 -> false
    | 2 ->
        (* this is the situation when the ban point has been put on the board.*)
        let w = Utils.get_w state.board in
        let all_cols = List.init w (fun x -> x) in
        let ban_top_ls =
          List.filter (fun col -> state.board.(0).(col) = -1) all_cols
        in
        if List.length ban_top_ls = 0 then
          ...
          let full_ban_ls =
            List.filter
              (fun col ->
                state.board.(1).(col) = 1 || state.board.(1).(col) = 2)
              ban_top_ls
          in
...
let is_terminal (state : state) : bool =
  ...
  else is_full_state state || check_winner_with_last state
...
let apply_action (state : state) (action : action) : state =
  ...
  | 1 ->
      new_board.(move_x).(move_y) <- -1;
      (* ban point be put into this move place*)
      {
        board = new_board;
        current_player = Utils.switch_player state.current_player;
        (* switch player!*)
        ...
        w = state.w;
        last_move = (move_x, move_y);
        (* !!NOTICE: this last move cannot be used for check_win_with_last*)
        ban_point = (move_x, move_y);
        (* NOTICE: ban point has been put*)
        bonus_point = (-1, -1);
        (* NOTICE: bonus has been eaten*)
        get_bonus = 2;
        (* switch to bonus state 2*)
      }
  | 2 ->
      new_board.(move_x).(move_y) <- state.current_player;
      {
