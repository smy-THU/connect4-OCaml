open Connect4
module ConnectFourAlpha = Alpha_beta.Make (Connect4)

let play_game rows cols player depth =
    ...
    if Connect4.is_terminal state then
      ...
      let next_state =
        if state.current_player = 1 then (
          ...
          let action = ConnectFourAlpha.search state depth in
...
let () =
  ...
  (*1 for human first, 2 for agent first*)
  let depth = 8 in
  play_game rows cols initial_player depth
