...
(** The type representing the game board, which is a 2D array of integers. 0
    represents an empty cell, 1 represents Player 1's piece, and 2 represents
    Player 2's piece. *)

type action = int
(** The type representing an action, which is the index of the column where a
...
type state = {
  ...
  last_move : point_t;
      (** The last move made, or (-1, -1) if no move has been made. *)
}
