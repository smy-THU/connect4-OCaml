...
module Make (G : Game) = struct
  ...
  let create_node ?(action = None) ?(parent = None) state =
    { state; action; wins = 0.0; visits = 0; children = []; parent }
  ...
  let ucb1 (child : node) (total_visits : int) (exploration : float) : float =
    if child.visits = 0 then infinity
      ...
      +. exploration
         *. sqrt
              (2.0
              *. log (float_of_int total_visits)
              /. float_of_int child.visits)

  let select_best_node (node : node) (exploration : float) : node =
    List.fold_left
      (fun best child ->
        if
          ucb1 child node.visits exploration > ucb1 best node.visits exploration
        then child
        else best)
  ...
  let backpropagate (node : node) (result : float) : unit =
    let rec propagate current =
      ...
      match current.parent with Some parent -> propagate parent | None -> ()
    in
    propagate node

  let search (root_state : G.state) (iterations : int) (exploration : float) :
      G.action =
    ...
    for _ = 1 to iterations do
      let rec select node =
        ...
        else select (select_best_node node exploration)
      in
    ...
    let best_child =
      List.fold_left
        (fun best child ->
          if float_of_int child.visits > float_of_int best.visits then child
          else best)
