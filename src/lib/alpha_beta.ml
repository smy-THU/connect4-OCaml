...
module Make (G : Game) = struct
  let rec minimax (state : G.state) (depth : int) (alpha : float) (beta : float)
      (maximizing : bool) : float * G.action option =
    if depth = 0 || G.is_terminal state then (G.evaluate state, None)
    else
      ...
      if maximizing then
        ...
        let rec max_value alpha best_action = function
          ...
          | action :: rest ->
              ...
              else max_value alpha best_action rest
        in
        ...
        let rec min_value beta best_action = function
          ...
          | action :: rest ->
              ...
              if value < beta then min_value (min beta value) (Some action) rest
              else min_value beta best_action rest
        in
  ...
  let search (state : G.state) (depth : int) : G.action =
    ...
    match action with Some a -> a | None -> failwith "No valid action found"
end
