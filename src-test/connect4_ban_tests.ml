...
let test_initial_state _ =
  ...
  assert_equal (state.board.(2).(3)) (-1); (* Ban point marked with -1 *)
...
let test_generate_actions_normal _ =
  ...
  let w = 7 in
  let state = initial_state h w 1 (3, 4) in
  assert_equal (generate_actions state) [0; 1; 2; 3; 4; 5; 6] (* All columns are available *)

let test_generate_actions_with_ban_point _ =
  let h = 6 in
  let w = 7 in
  let state = initial_state h w 1 (2, 3) in
  assert_equal (generate_actions state) [0; 1; 2; 3; 4; 5; 6] (* All columns should still be valid, except dropping on (2, 3) *)
...
let test_apply_action_valid_move _ =
  ...
  assert_equal (new_state.board.(5).(3)) 1 (* Player 1 drops piece on the 3rd column *)

let test_apply_action_invalid_column _ =
  let h = 6 in
  let w = 7 in
  let state = initial_state h w 1 (2, 3) in
  assert_raises (Failure "Invalid column index") (fun () -> apply_action state (-1));
  assert_raises (Failure "Invalid column index") (fun () -> apply_action state 7)

let test_apply_action_column_full _ =
  let h = 6 in
  let w = 7 in
  let state = initial_state h w 1 (2, 3) in
  Array.fill state.board.(0) 0 7 1;  (* making column full *)
  assert_raises (Failure "Column is full, no space to drop") (fun () -> apply_action state 3)
...
let test_apply_action_with_ban_point _ =
  ...
  let new_state = apply_action state 3 in
  assert_equal (new_state.board.(4).(3)) 1; (* drop above the ban point *)
  let state = initial_state h w 1 (4, 3) in
  let state = apply_action state 3 in
...
let series =
  "Connect4_ban Tests" >::: [
    ...
    "test_check_winner_with_last_no_winner" >:: test_check_winner_with_last_no_winner;
    ...
    "test_generate_actions_with_ban_point" >:: test_generate_actions_with_ban_point;
