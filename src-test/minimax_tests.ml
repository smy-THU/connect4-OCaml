...
let test_minimax_terminal _ =
  ...
  assert_equal ~msg:"Terminal state evaluation" ~printer:string_of_float 0.0 score

let test_minimax_non_terminal _ =
  let state = 5 in
  let score = Minimax.minimax state 3 true in
  assert_equal ~msg:"Non-terminal state evaluation" ~printer:string_of_float 1.0 score
